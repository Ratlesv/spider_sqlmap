...
class Agent(object):
    ...
    def payload(self, place=None, parameter=None, value=None, newValue=None, where=None):
        ...
        if place == PLACE.URI or BOUNDED_INJECTION_MARKER in origValue:
            ...
            if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                ...
                origValue = extractRegexResult(r"(?s)\"\s*:\s*(?P<sqlmapapi_result>\d+\Z)", origValue) or extractRegexResult(r'(?s)[\s:]*(?P<sqlmapapi_result>[^"\[,]+\Z)', origValue)
            else:
                _ = extractRegexResult(r"(?s)(?P<sqlmapapi_result>[^\s<>{}();'\"&]+\Z)", origValue) or ""
    ...
    def extractPayload(self, value):
        ...
        return extractRegexResult(r"(?s)%s(?P<sqlmapapi_result>.*?)%s" % (_, _), value)
